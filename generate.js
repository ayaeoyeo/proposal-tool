export default async function handler(req, res) {
  ...
  const whysoNote = whyso
    ? whyso
    : `(데이터 없음 → ${gu} 청년 관련 통계/연구 기반으로 설득력 있는 근거를 직접 작성해줘. 예: 고립·은둔 위험군 비율, 1인가구 증가, 관계망 붕괴 등 실증적 맥락 활용)`;

  const prompt = `당신은 서울시 주민참여예산 제안 신청서 전문 작성가입니다.
아래 입력 정보를 바탕으로 완성도 높은 주민참여예산 제안 신청서를 작성해주세요.

[핵심 작성 원칙]
- 사업명: "대상 → 목적 → 방법 → 부제" 구조 (예: 관악구 청년 1인가구의 안전한 관계망 형성을 위한 공공기반 이성 매칭 프로그램 '4가지 없는 소개팅')
...
- "결혼 장려"보다 "관계 형성·연결 기회 제공"으로 프레이밍
...
다음 JSON 형식으로만 응답해줘. 마크다운 기호나 다른 텍스트 없이 순수 JSON만:
{
  ...
  try {
    ...
    return res.status(500).json({ error: '신청서 생성 중 오류가 발생했어요. 다시 시도해주세요.' });
  }
}
